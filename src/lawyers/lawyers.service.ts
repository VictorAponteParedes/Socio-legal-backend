...
export class LawyersService {
  ...
  async completeProfile(
    ...
    if (lawyer.profileCompleted) {
      throw new BadRequestException(
        'El perfil ya está completado. Usa el endpoint de actualización.',
      );
    }
  ...
  async searchLawyers(filters: {
    ...
    if (filters.search) {
      ...
      queryBuilder.andWhere(
        '(LOWER(user.name) LIKE LOWER(:search) OR LOWER(user.lastname) LIKE LOWER(:search))',
        { search: searchTerm },
      );
    }

    if (filters.specializationIds && filters.specializationIds.length > 0) {
      queryBuilder.andWhere('specializations.id IN (:...specializationIds)', {
        specializationIds: filters.specializationIds,
      });
    }

    if (filters.city) {
      queryBuilder.andWhere('LOWER(lawyer.city) = LOWER(:city)', {
        city: filters.city,
      });
    }

    if (filters.country) {
      queryBuilder.andWhere('LOWER(lawyer.country) = LOWER(:country)', {
        country: filters.country,
      });
    }

    queryBuilder.orderBy('lawyer.rating', 'DESC');
    queryBuilder.addOrderBy('lawyer.totalReviews', 'DESC');
  ...
  async checkProfileCompletion(
    userId: string,
  ): Promise<{ completed: boolean; missing: string[] }> {
    const lawyer = await this.getMyProfile(userId);
